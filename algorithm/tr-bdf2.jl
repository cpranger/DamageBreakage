...
function step!(i::tr_bdf2; newton_maxit, newton_atol)
	γ   = 2/1 - sqrt(2)    # = 0.5857864376269049
	a32 = 1/2 + sqrt(2)/3  # = 0.9714045207910318
	...
	assign!(i.e, i.w[2])#i.y + i.h_t * γ * (i.f_ex(i.w[1]) + i.f_im(i.w[1])))
	assign!(i.w[2], i.y + i.h_t * (
	    (2/1-sqrt(2)/1) * i.f_ex(i.w[1])
	  + (1/1-sqrt(2)/2) * i.f_im(i.w[1]) + (1/1-sqrt(2)/2) * i.f_im(i.w[1])
	))
    ...
    assign!(i.e, i.w[3])#i.y + i.h_t * (1-a32)*(i.f_im(i.w[1]) + i.f_ex(i.w[1])) + i.h_t * (  a32) * (i.f_im(i.w[2]) + i.f_ex(i.w[2])))
	assign!(i.w[3], i.y + i.h_t * (
	    (1/2-sqrt(2)/3) * i.f_ex(i.w[1]) + (1/2+sqrt(2)/3) * i.f_ex(i.w[2])
	  + (    sqrt(2)/4) * i.f_im(i.w[1]) + (    sqrt(2)/4) * i.f_im(i.w[2]) + (1/1-sqrt(2)/2) * i.f_im(i.w[2])
	))
